...
//请求直播推荐接口
        ...
        if ($version > 5) {
            ...
            //$sqlb = "SELECT  COUNT(u.aid) as num,u.aid ,title  FROM `dede_appsoulist` l   , `dede_aasou` u   where l.id=u.aid  group by u.aid    order by COUNT(u.aid) desc limit  8 ";
...
//            $total=$row['count'];
            ...
            foreach ($outlisttype as $v) {
                ...
                $sqlc = M('pagetype')->where("aid = " . $v['id'])->select();
