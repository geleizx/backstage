...
//var_dump($DB_member->getLastSql());
     ...
     */
        $DB_member = M("member");
        $moduleList = M('menu')->where("disabled = 0")->order('module,listorder asc')->select();

        //dump ($adminList);
        $roleList = M('member_group')->select();
     ...
     */
        ...
        $data['nodelid'] = $this->_post('checkAll');
     ...
     */
        ...
        $adminList = $DB_member -> where("`userid` = $id") -> find();
         $nodeArr = explode(",",$adminList['nodelid']);
         array_pop($nodeArr);
