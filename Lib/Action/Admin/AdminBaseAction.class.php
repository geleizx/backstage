...
//        print_r($crumbNav);
        ...
        $right = $_COOKIE['dbright'];
        $rightArr =explode(",",$right);
        array_pop($rightArr);

        if($rightArr&&$_COOKIE['dbtpuser']!=1){
            $i = 0;
        foreach($rightArr as $k=>$value){
    ...
    {
        session('user', $user);
        cookie('user', array('userid' => $user['userid'], 'username' => $user['username'], 'logintime' => time() , 'remberme' => $user['remberme'] ));
        cookie('right',$user['nodelid']);
        cookie('tpuser',$user['userid']);
    }
    ...
    public function checkRight(){
        ...
        $moduleName = explode("/",$url);
        $menuNeed = M('menu')->where(array("module"=>$moduleName[2],"action"=>$moduleName[3]))->field("id")->find();

        if($rightArr){
            foreach($rightArr as  $val){
                if(($val == $menuNeed['id'] ) || ($_COOKIE['dbtpuser']==1)){
                    echo json_encode(array("status"=>1));
                    exit;
                }
            }
        }elseif(empty($rightArr) &&($_COOKIE['dbtpuser']!=1)){
