...
class InterfaceAction extends Action{
    ...
    public function recommendedChannelData(){
        $sqls = "SELECT  catid ,catname ,image,status FROM `tp_category` where status = 2 limit 8";
        //$list = $categoryModel->where($where)->field("catid,catname,image,status")->limit(0,8)->select();
        $BaseAction =A("Admin/Base");
        $list = $BaseAction->postmem($sqls,$type = "one");
        foreach($list as $k=>$val){
            $id = $list[$k]["catid"];
            $sqlsub = "SELECT product_name,type,param1,param2,appid FROM `tp_product` where category_id = $id and status =1 order by  sort asc";
            $subContent = $BaseAction->postmem($sqlsub,$type = "one");
            //$subContent = M('product')->where("category_id = $id and status=1")->field("product_name,type,param1,param2,appid")->order("sort asc")->select();
    ...
    public function requestChannelName(){
        ...
        if($catid ==99||$screen==1){
            ...
            if($version<10&&$version){
                $str = "parentid=0 and status !=0 and  module=2 and screening = 0";
            }else{
                 $str = "parentid=0 and status !=0 and  module=2 and screening = 0 and catid != 2013";
            }
            $catList = $category->where($str)->field("catid,catname,screening")->order("listorder asc")->select();
