...
class ProductAction extends AdminBaseAction {
	...
	public function index(){
        ...
        $dsm = M('product')->where("product_name = '电视猫直播'")->count();
        ...
        $this->assign('vst',$vst);
        $this->assign('dsm',$dsm);
        $result =$category->where("catid = $catid")->find();

        if($result['parentid']==0&&$catid){
            $this->assign("tpl",1);
            $category = $this->queryCategory($catid, 0, 2);
...
//            vendor("PHPExcel.PHPExcel");
            ...
            for($i=2;$i<=$highestRow;$i++)
            {
                ...
                $data['category_id'] = $objPHPExcel->getActiveSheet()->getCell("C".$i)->getValue();
                $data['param1'] = $objPHPExcel->getActiveSheet()->getCell("G".$i)->getValue();
                $data['param2'] = $objPHPExcel->getActiveSheet()->getCell("E".$i)->getValue();
