...
class IndexAction extends AdminBaseAction{
    ...
    public function index(){
        ...
        $testList = $test->select();
            foreach($testList as $k=>$v){
                ...
                $enddate = time();
                $startdate = strtotime(filemtime($fileName));
                $date=floor(($enddate-$startdate)/86400);

                $testList[$k]['TimeDiff'] = $date;
                $testList[$k]['newsSize'] =  filesize($fileName);
                $testList[$k]['newsTime'] =  filemtime($fileName);
            }

        //更新点击率
        $res = M('json')->order("id desc")->field('content')->select();
        for($i=0;$i<count($res);$i++){
        if($res[$i]['status']==0) {
            $json = htmlspecialchars_decode($res[$i]['content']);
            $result = json_decode($json, 1);
            foreach ($result['vodList'] as $vod) {
                $update = M('page')->where(array("id" => $vod['id'], "title" => $vod['catname']))->setInc("hits", $vod['hits']);
            }
            foreach ($result['liveList'] as $live) {
                $update2 = M('category')->where(array("catid" => $live['id'], "catname" => $live['catname']))->setInc("hits", $vod['hits']);
            }
            if ($update && $update2) {
                $where['status'] = 1;
                $changeStatus = M('json')->where("id =" . $res[$i]['id'])->update($where);
            }
        }
        }
    ...
    public function getCat(){
        $map['id']  = array('between','28560,28659');
        $film = M('page')->where($map)->field("title,id")->limit(0,100)->select();
//        foreach($film as $key=>$value){
//            $key = $value['title'];
//            $result = file_get_contents("http://cdn.91vst.com/api/dirsearch.action?topId=0&type=4&key=".urlencode($key)."&pageSize=14015&pageNo=1&version=2.6&videoType=0");
//            $content = json_decode($result,1);
//
//            if($content){
//                $getTitle = $content['data']['video'][0]['title'];
//                $getCat = $content['data']['video'][0]['cat'];
//                $data['cat'] = $getCat;
//                $insert = M('page')->where("id = " .$value['id'])->save($data);
//            }
//        }
